...
int pulse_demod_pcm(const pulse_data_t *pulses, r_device *device)
{
    ...
    for (unsigned n = 0; n < pulses->num_pulses; ++n) {
        ...
        // End of Message?
        //SerialPrintLn("Test for end of message");
        if (((n == pulses->num_pulses - 1)                            // No more pulses? (FSK)
            ...
            /*SerialPrintMsg("Num rows = ");
            SerialPrint(bits.num_rows);
            SerialPrintLn("");*/

            //SerialPrintMsg(device->name);

            events += account_event(device, &bits, __func__);

            /*SerialPrintMsg(" - free stack = ");
            SerialPrint(uxTaskGetStackHighWaterMark(NULL));
            SerialPrintLn("");*/
...
{
    //SerialPrintLn("pulse_demod_ppm begin");
    ...
    //SerialPrintLn("pulse_demod_ppm check for rounding");
    ...
    //SerialPrintLn("pulse_demod_ppm check for tolerance");
    if (s_tolerance > 0) {
        // precise
    ...
    //SerialPrintLn("pulse_demod_ppm process pulses");
    for (unsigned n = 0; n < pulses->num_pulses; ++n) {
